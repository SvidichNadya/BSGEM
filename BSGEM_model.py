...
# =============================================================================
# FJ Агентная динамика
# =============================================================================
class FJAgentDynamics(nn.Module):
    def __init__(self, n_agents=1, init_opinion=0.5, learnable=True):
        super().__init__()
        self.n_agents = n_agents
        if learnable:
            self.lambda_ = nn.Parameter(torch.tensor(0.5, dtype=torch.float32))
            self.alpha = nn.Parameter(torch.tensor(0.3, dtype=torch.float32))
            self.gamma = nn.Parameter(torch.tensor(0.5, dtype=torch.float32))
            self.kappa_self = nn.Parameter(torch.tensor(0.2, dtype=torch.float32))
        else:
        ...
        self.register_buffer('m0', torch.tensor(init_opinion))

    def forward(self, external_field, prev_opinion=None, neighbor_mean=None):
        if prev_opinion is None:
            prev_opinion = self.m0
        if neighbor_mean is None:
            neighbor_mean = prev_opinion
        numerator = self.lambda_ * neighbor_mean + self.alpha * self.m0 + self.gamma * external_field
        denominator = 1.0 + self.alpha
        new_opinion = numerator / denominator
        confidence = torch.sigmoid(1.0 - torch.var(prev_opinion))
        self_deception = self.kappa_self * (prev_opinion - neighbor_mean) * confidence
        final_opinion = torch.clamp(new_opinion + self_deception, 0.0, 1.0)
...
class AgentPopulation:
    ...
    def utility(self, c, s, anxiety, alpha, beta, gamma):
        return alpha * np.log(c + 1e-6) + beta * np.log(s + 1e-6) - gamma * anxiety

    def compute_mpc(self, anxiety):
        mpc_sum = 0.0
        for i in range(self.n_agents):
            ...
            for c in np.linspace(0, 1, 20):
                s = 1.0 - c
                u = self.utility(c, s, anxiety, self.alphas[i], self.betas[i], self.gammas[i])
                if u > best_u:
                    best_u = u
                    best_c = c
            mpc_sum += best_c
...
# =============================================================================
# Causal Graph (раздел 6.1) – фиксированная топология
# =============================================================================
class CausalGraph(nn.Module):
    def __init__(self, n_nodes=6):
        ...
        self.edge_weights = nn.ParameterDict({
            f"{i}->{j}": nn.Parameter(torch.tensor(1.0)) for (i,j) in edges
        })
    def forward(self, x):
        return x

# =============================================================================
# HMM на скользящем окне (без будущего)
# =============================================================================
...
# =============================================================================
# Фильтр Калмана онлайн (только вперед)
# =============================================================================
...
# =============================================================================
# Упрощенная модель BSGEM (без VAE, только GRU + эндогенное поле + HFE)
# =============================================================================
class BSGEM_Deterministic(nn.Module):
    def __init__(self, input_dim, hidden_dim=8, kappa_self=0.2, omega=0.1):
        super().__init__()
        self.omega = omega
        self.fj = FJAgentDynamics(n_agents=1, init_opinion=0.5, learnable=True)
        self.causal_graph = CausalGraph(n_nodes=6)
        self.gru = nn.GRU(input_dim + 1, hidden_dim, batch_first=True, num_layers=1, dropout=0.0)
    ...
    def forward(self, x, return_attention=False):
        ...
        # FJ динамика для получения эндогенного поля
        external_field = x[:, :, 3]
        opinions = []
        prev_opinion = None
        for t in range(seq):
            field = external_field[:, t]
            new_opinion = self.fj(field, prev_opinion)
            if torch.isnan(new_opinion).any():
                new_opinion = torch.zeros_like(new_opinion)
            opinions.append(new_opinion.unsqueeze(1))
            prev_opinion = new_opinion
        opinions = torch.cat(opinions, dim=1)
        opinions = torch.clamp(opinions, -1.0, 1.0)
        endog_field = self.omega * opinions.unsqueeze(-1)

        # Добавляем эндогенное поле к входным признакам
        x_aug = torch.cat([x, endog_field], dim=-1)
        x_aug = torch.nan_to_num(x_aug, nan=0.0)
        gru_out, _ = self.gru(x_aug)
        attn_weights = torch.softmax(self.attn(gru_out), dim=1)
        context = torch.sum(attn_weights * gru_out, dim=1)
        y_pred = self.fc_out(self.dropout(context))
        if return_attention:
            return y_pred, attn_weights, opinions
        return y_pred

# =============================================================================
# Обучение детерминированной GRU с ранней остановкой по R²
# =============================================================================
def train_bsgem_det(model, X_train, y_train, X_val, y_val,
    ...
    for epoch in range(epochs):
        ...
        for batch_x, batch_y in loader:
            ...
            valid_batches += 1
        if valid_batches == 0:
            print(f"Epoch {epoch+1}: all batches NaN, skipping")
            continue
        ...
        if val_r2 > best_val_r2:
            ...
            if patience_counter >= patience:
                ...
                break
        if (epoch+1) % 10 == 0:
            print(f"Epoch {epoch+1}/{epochs} - Train Loss: {epoch_loss:.4f}, Val R²: {val_r2:.4f}")

    model.load_state_dict(best_state)
    return model, train_losses, val_losses, val_r2s

# =============================================================================
# Классы для извлечения признаков (InformationField, RussianNLI, NarrativeExtractor, SocialSuggestibilityIndex)
# =============================================================================
...
# =============================================================================
# Извлечение признаков без утечки будущего
# =============================================================================
def extract_features_no_future(texts, timestamps, window_hmm=100):
    ...
    # Information field (только прошлые расстояния)
    ...
    # Динамическое MPC
    agent_pop = AgentPopulation(n_agents=2000)
    ...
    for i in range(len(texts)):
        ...
        mpc_series[i] = agent_pop.compute_mpc(avg_anx)
    ...
    return X, {'info_gradient': F_info}

# =============================================================================
# Бенчмарки (Transformer, DeepAR) и CausalEffectEstimator
# =============================================================================
...
def main(csv_file, output_json='report.json'):
    ...
    print("Extracting features (no future leaks)...")
    ...
    for fold, (train_idx, test_idx) in enumerate(tscv.split(X_seq)):
        ...
        # ---- BSGEM-HFE (детерминированная GRU) ----
        # Базовая модель без HFE
        base_model = BSGEM_Deterministic(input_dim=X_train.shape[2], hidden_dim=8,
                                         kappa_self=0.0, omega=0.0)
        base_model, _, _, _ = train_bsgem_det(base_model, X_train, y_train, X_test, y_test,
                                              epochs=60, patience=10, lr=1e-4)
        base_model.eval()
        with torch.no_grad():
            X_train_t = torch.tensor(X_train, dtype=torch.float32).to(device)
            y_pred_base_train = base_model(X_train_t).cpu().numpy().flatten()
            residuals = y_train - y_pred_base_train
            X_test_t = torch.tensor(X_test, dtype=torch.float32).to(device)
            y_pred_base_test = base_model(X_test_t).cpu().numpy().flatten()
        ...
        final_model = BSGEM_Deterministic(input_dim=X_train_aug.shape[2], hidden_dim=8,
                                          kappa_self=0.3, omega=0.1)
        final_model, train_losses, val_losses, val_r2s = train_bsgem_det(
            final_model, X_train_aug, y_train, X_test_aug, y_test,
            epochs=120, patience=30, lr=1e-4)
        ...
        with torch.no_grad():
            ...
            for _ in range(30):
                final_model.train()
                y_pred_s = final_model(X_test_aug_t).cpu().numpy().flatten()
                y_pred_samples.append(y_pred_s)
    ...
    print("\n" + "="*80)
    print("BSGEM-HFE (deterministic GRU) results across folds")
    print("="*80)
    ...
    print(f"MSE={np.mean(mse_vals):.4f}±{np.std(mse_vals):.4f}  "
          f"MAE={np.mean(mae_vals):.4f}±{np.std(mae_vals):.4f}  "
          f"R²={np.mean(r2_vals):.4f}±{np.std(r2_vals):.4f}")
    print(f"Average CATE (75th percentile info shock) over folds: {np.mean(all_cates):.4f}±{np.std(all_cates):.4f}")
    ...
    if best_y_pred is not None:
        ...
        plt.figure(figsize=(12,5))
        plt.plot(best_train_losses, label='Train loss (MSE)')
